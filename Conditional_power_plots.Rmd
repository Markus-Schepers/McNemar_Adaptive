...
rowwise() %>%
mutate(cp = conditional_power(p_star = p, psi=0.2, nd1 = nd1, nsecond = 38)) %>%
ungroup()

p1 <- ggplot(df_cp, aes(x = p, y = nd1, fill = cp)) +
geom_tile() +
scale_fill_viridis_c() +
labs(title = "Conditional Power (fixed second stage sample size = 38)",
x = expression(p["*"]), y = expression(n["d"]^"(1)")) +
theme_minimal()
...
rowwise() %>%
mutate(n2min = min_nsecond(p_star = p, nd1 = nd1,
